...
df = pd.read_csv("myresults/pubmed_dataframe_alltextmining.csv")

possible_lists = []
f = open("intersections/pubmed_possible_lists", encoding="utf-8")
lines = f.readlines()
for line in lines:
    possible_lists.append(int(line.strip("\n")))


relevant_lists = []
f = open("intersections/pubmed_relevant_lists", encoding="utf-8")
lines = f.readlines()
for line in lines:
    relevant_lists.append(int(line.strip("\n")))
...
plt.scatter(relevant_lists_FORM_mir_score, relevant_lists_tax_score, color='green', label='relevant_lists', alpha=0.5, s=5)
#plt.scatter(difference_FORM_mir_score, difference_tax_score, color='red', label='difference', alpha=0.5, s=5)
#plt.scatter(relevant_lists3_FORM_mir_score, relevant_lists3_tax_score, color='yellow', label='relevant_lists3', alpha=0.5, s=10)
